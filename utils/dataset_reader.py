...
def read_tracks(filename,case_id=1):
    with open(filename) as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        
        temp  = list(csv_reader)
        bb,aa = temp[0],pd.DataFrame(temp[1:],columns = temp[0])
        aa[['case_id','track_id','frame_id']] = aa[['case_id','track_id','frame_id']].values.astype(float)
        aa = aa[(aa['case_id']==case_id) & (aa['agent_type']=='car')] .sort_values(by = ['track_id','frame_id'],axis = 0,ascending = True)
        #aa[['case_id','track_id','frame_id']] = aa[['case_id','track_id','frame_id']].values.astype(str)
        del aa['case_id']
        ...
        for i, row in enumerate(temp):
            ...
            track.motion_states[ms.frame_id] = ms
