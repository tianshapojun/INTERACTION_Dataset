...
def vectorize_ego(track_id, history_num_ego_frames, history_track,current_track,future_track):
    ...
    agent_extent_m = np.array([agent['length'].iloc[0],agent['width'].iloc[0]])
    agent_centroid_m = np.array([agent['x'].iloc[0],agent['y'].iloc[0]])
    agent_yaw_rad = np.array(agent['psi_rad'].iloc[0])
    
    history = history_track[history_track['track_id'] == track_id].sort_values(by = ['frame_id'],axis = 0,ascending = False)
    history_coords = np.array(history[['x','y']])
    history_yaws = np.array(history['psi_rad'])
    ...
    future_coords = np.array(future[['x','y']])
    future_yaws = np.array(future['psi_rad'])
